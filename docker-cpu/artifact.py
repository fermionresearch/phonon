"""Portable Phonon fold4 artifact conversion for the CPU runtime.
...
MLX, producing the row-major nibble stream consumed by the kernel ABI.
...
def load_fold4_matrix(model_dir: str | Path, module_name: str) -> Fold4Matrix:
    """Load one decoder module and derive its fold4 runtime matrix."""
    ...
    if decoder_codes.get("format") != "ten-base5-per-24bit-v1":
        raise ValueError("fold4 loader requires ten-base5-per-24bit-v1")
    if metadata.get("format") != "broadcast-scales-v1":
        raise ValueError("fold4 loader requires broadcast-scales-v1")
    if manifest.get("group_size") != 128:
        raise ValueError("fold4 loader requires group_size=128")
    module = next(
