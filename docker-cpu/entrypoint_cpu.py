#!/usr/bin/env python3
"""phonon-cpu — Phonon-1 on plain CPUs.

Two modes:

  phonon-cpu transcribe <audio...> [--model NAME] [--model-dir DIR]
  ...
  phonon-cpu serve [--host 127.0.0.1] [--port 8000] [--api-key KEY]
...
All three published models run here: `--model phonon-1` (the default),
`--model phonon-1-big`, `--model phonon-1-micro`. Without `--model-dir`
the model's release archive is downloaded from Hugging Face, verified
against its published SHA-256 pin, and unpacked; with `--model-dir` a
local copy is used and its manifest must match the requested model.

The execution path IS the transcript-gated packed CPU configuration
(greedy decode, temperature 0.0, max 512 new tokens, no repetition
penalty) — the configuration behind the published CPU accuracy numbers.
There is no experimental opt-in flag in this image: the packed CPU path is
the gated path. Long audio and streams are segmented (never resampled,
never truncated) and every segment is decoded by that same configuration.

Requires an x86-64 CPU with AVX2 (any mainstream CPU from roughly 2014 on).
"""
...
#: artifact storage layout the engine verifies against the directory.
CATALOG = {
    "phonon-1": {
        "name": "Phonon-1",
...
GENERIC_MODEL_NAMES = {"", "phonon", "phonon-cpu"}
...
def resolve_model(args) -> tuple[str, Path]:
    ...
    if args.model_dir:
        ...
        try:
            claimed = (CATALOG[resolve_model_name(args.model)]["profile"]
                       if args.model else None)
            profile = engine.resolve_profile(model_dir, claimed)
        except ValueError as exc:
            fail(str(exc))
        return profile_to_model(profile), model_dir

    key = resolve_model_name(args.model)
    entry = CATALOG[key]
    import _archive
...
class Transcriber:
    """Loads the model once; every decode is the gated packed CPU path."""
    ...
    def __init__(self, model_key: str, model_dir: Path):
        ...
        self.engine = engine.CpuSpeech(
            model_dir, profile=self.entry["profile"], log=log)
    ...
    def load_s(self) -> float:
        return self.engine.load_s

    def transcribe(self, wav) -> str:
        """One gated decode of <= 30 s of audio."""
        return self.engine.transcribe(wav)

    def segment_decode(self, wav, lock: threading.Lock | None = None) -> str:
        """One session-segment decode: near-silence short-circuit (a segment
        whose peak is below 1e-4 holds nothing worth decoding), then the
        gated decode, optionally behind the engine lock."""
        import numpy as np
    ...
    def transcribe_long(self, wav, *, lock: threading.Lock | None = None,
        ...
        <= 30 s: exactly the single gated decode. Longer: energy-gated
        segmentation (the Mac engine's constants), each segment decoded by
        the gated configuration, finals joined with single spaces.
        Returns (text, segment_count).
        """
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _transcriptions(self) -> None:
        ...
        try:
            ...
            text, segments = self.engine.transcribe_long(
                wav, lock=self.server.decode_lock)  # type: ignore[attr-defined]
    ...
    def _stream(self, query: str) -> None:
        ...
        JSON events back; one stream at a time per worker (close 1013);
    ...
    def _stream_session(self, send_event, close) -> None:
        ...
        session = self.engine.session(
            ...
            lock=srv.decode_lock)  # type: ignore[attr-defined]
...
def cmd_serve(args) -> None:
    ...
    server.decode_lock = threading.Lock()  # type: ignore[attr-defined]
...
def _add_model_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "--model", default=None,
        help=f"which published model to run: phonon-1 (default), "
             f"phonon-1-big, or phonon-1-micro (aliases and repo ids "
             f"accepted)")
    parser.add_argument(
        "--model-dir",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog=BRAND,
        description="Phonon-1 on plain CPUs (x86-64, AVX2). The gated "
                    "packed decode path; long audio is segmented by the "
                    "Mac engine's energy gate.")
