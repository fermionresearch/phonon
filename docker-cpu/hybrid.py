...
fold4 modules, broadcast scales, group 128 — the loader in `artifact.py`)
...
``hybrid=False`` for it and `model.py` keeps the plain state loader.
"""
...
def hybrid_artifact_state(model_dir: str | Path) -> dict[str, torch.Tensor]:
    ...
    Mirrors `graph._artifact_state` (same key prefixing, same conv2d
